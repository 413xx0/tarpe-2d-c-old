...
static inline void tick_velocities(struct rigidbody * rb, float_t dt)
{
	struct vec2 tmp;
...
static inline void tick_pos_angle(struct rigidbody * rb, float_t dt)
{
	struct vec2 tmp;
...
int tarpe_tick(struct rbs_array * bodies, float_t dt)
{
	...
	for (struct rb_shape_base ** rbs_ptr = bodies->shapes; rbs_ptr < bodies->shapes + bodies->size;
	     ++rbs_ptr)
	{
		tick_velocities((struct rigidbody *)(*rbs_ptr), dt);
	}
	...
	for (struct rb_shape_base ** rbs_ptr = bodies->shapes; rbs_ptr < bodies->shapes + bodies->size;
	     ++rbs_ptr)
	{
		tick_pos_angle((struct rigidbody *)(*rbs_ptr), dt);
