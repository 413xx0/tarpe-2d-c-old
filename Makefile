...
LIBTARPE2D_DRAW_CLEAN_GOAL = clean

LIBTARPE2D_DRAW_INCULDE_DIRS = $(LIBTARPE2D_DRAW_PATH)/include
LIBTARPE2D_LDFLAGS = 
LIBTARPE2D_LDLIBS = -lSDL2 -lSDL2_gpu -lbsd
...
libtarpe2d_draw_build: $(LIBTARPE2D_DRAW_PATH)
	$(MAKE) -C $< OPT="$(OPT)" WARN_CONF="$(WARN_CONF)" STD="$(STD)" CC=$(CC) $(LIBTARPE2D_DRAW_BUILD_GOAL)
...
clean:
	...
	$(MAKE) -C $(LIBTARPE2D_DRAW_PATH) $(LIBTARPE2D_DRAW_CLEAN_GOAL)
